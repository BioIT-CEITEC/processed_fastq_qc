...
if os.stat(snakemake.input.in_filename).st_size != 0:
    ...
    unzip_out_R1 = snakemake.output.R1[:-3]
    unzip_out_R2 = snakemake.output.R2[:-3]
    ...
    R2_ins_start = config["insert_R2_start"]
        ...
        if config["UMI_from_R3_file"]:
            ...
            if config["UMI_write_to"] == "sep_file":
                ...
                command = "gunzip -c " + in_filename_UMI + " > " + umi_file + "; " +\
                          "mv -T " + in_filename + " " + snakemake.output.R1 + "; " +\
                ...
                command = "(paste <(zcat " + in_filename + ") <(zcat " + in_filename_R2 + ") <(zcat " + in_filename_UMI + ") |" + \
                          " awk '{{ if(NR%4==1) {{split($1,head_R1,\"" + sep + "\"); split($2,head_R2,\"" + sep + "\")}}" + \
                          " else if(NR%4==2) {{print head_R1[1] \"_\" $3 \" \" head_R1[2] \"\\n\" substr($1,7) > out1;" + \
                          " print head_R2[1] \"_\" $3 \" \" head_R2[2] \"\\n\" substr($2,7) > out2}} else if(NR%4==0) {{print substr($1,7) > out1;" + \
